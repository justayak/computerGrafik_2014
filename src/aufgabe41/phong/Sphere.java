...
public class Sphere {
	...
	public Vector3[] normal(int x, int y) {
		...
		// T.subtract(this.position).normalize());
		return new Vector3[] { T.subtract(this.position).normalize(), T };

	}

	/**
	 * test, if this Sphere can be visible at pixel (x,y)
	 * 
	 * @param x
	 * @param y
	 * @return
	...
	public Vector3[] IntersectionHits(Ray ray){
		if (ray.source.distance(position) < radius) {
		...
		if (hits.length == 0) {
			...
			return new Vector3[]{ray.pointAt(hits[0])};
		} else {
			return new Vector3[]{ray.pointAt(hits[0]), ray.pointAt(hits[1])};
		}
	}
	...
	public boolean intersect(Ray r) {
		...
		Vector3 dst = new Vector3(this.position.x - eye.x, this.position.y
				- eye.y, this.position.z - eye.z);
	...
	private double p(Vector3 posToSource, Ray ray) {
		return 2 * (posToSource.x * ray.direction.x + posToSource.y
				* ray.direction.y + posToSource.z * ray.direction.z);
	}
